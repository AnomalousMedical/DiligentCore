    ...
    TPipelineStateBase{pRefCounters, pDeviceD3D12, CreateInfo}
{
    try
    {
        ...
        if (m_Desc.PipelineType == PIPELINE_TYPE_GRAPHICS)
        {
            ...
            const bool  IsCreatedFromCache = (pPSOCacheD3D12 != nullptr && !WName.empty() && pPSOCacheD3D12->LoadGraphicsPipeline(WName, d3d12PSODesc, m_pd3d12PSO));

            if (!IsCreatedFromCache)
            {
                ...
                if (pPSOCacheD3D12 != nullptr && !WName.empty())
                    pPSOCacheD3D12->StorePipeline(WName, m_pd3d12PSO);
            }
        }
    ...
    TPipelineStateBase{pRefCounters, pDeviceD3D12, CreateInfo}
{
    try
    {
        ...
        const auto  WName              = WidenString(m_Desc.Name);
        auto* const pPSOCacheD3D12     = ClassPtrCast<PipelineStateCacheD3D12Impl>(CreateInfo.pPSOCache);
        const bool  IsCreatedFromCache = (pPSOCacheD3D12 != nullptr && !WName.empty() && pPSOCacheD3D12->LoadComputePipeline(WName, d3d12PSODesc, m_pd3d12PSO));
